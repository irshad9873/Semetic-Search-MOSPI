...
class LlamaReranker:
    def __init__(
        ...
        # Load and flatten datasets + indicators + filters
                        ...
                        for fname, options in f.items():
                            for opt in options:
                                self.products.append({
                                    ...
                                    "name": fname,           # Filter name like "State"
                                    "desc": opt,             # Filter option like "Delhi"
                                    "type": "filter",
                                    "parent": ind_code,
                                    "filter_name": fname,
                                    "option": opt
    ...
    def _build_choice_block(self, candidates: List[Dict[str, Any]]) -> str:
        ...
        for i, p in enumerate(candidates, start=1):
            lines.append(
                f"{i}. CODE={p.get('code','UNKNOWN')}\n"
                f"   NAME={p.get('name','UNKNOWN')}\n"
                f"   DESC={p.get('desc','No description')}\n"
                f"   TYPE={p.get('type','')}\n"
            )
        return "\n".join(lines)
